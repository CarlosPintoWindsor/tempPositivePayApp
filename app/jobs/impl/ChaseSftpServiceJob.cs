...
{
    class ChaseSftpServiceJob(
        AppConfig appConfig,
        [FromKeyedServices("ChaseSFTP")] SftpService sftpService,
        ILogger logger,
        ...
        ) : IProcessorJob
    {
        ...
        public void ProcessFile(FileStream fileStream, string fileName)
        {
            try
            {
                string destinationFolder = appConfig.Jobs["Chase"].DestinationFolder;
                string finalFileName = this.TransformFileName(fileName);
                sftpService.Connect();

                logger.LogInformation($"Dropping file [{fileName} into Chase sftp");
